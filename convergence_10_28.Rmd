...
The three growth regimes are defined by the following country codes (first split at 1960 GDP per capita of $863, then 1950 literacy rate of 45%, per Hansen [2000]):

```{r}
# Column 1
group1 = c(
  "BFA","BDI","ETH","MWI","MLI","MRT","NER","RWA","SLE","TZA",
  "TGO","UGA","COD","MMR","CAF","LBR","BGD","NPL")
ineq1 = ineq |> filter(code %in% group1)

# Column 2
group2 = c(
  "DZA","AGO","BEN","CMR", "TCD","COG","EGY","GHA","CIV",
  "KEN","MAR","MOZ","NGA","SEN","SOM","SDN","TUN","ZMB",
  "ZWE","IND","JOR","PAK","SYR","TUR","GTM","HTI",
  "HND","BOL","IDN","PNG")
...
group3 = c(
  ...
  "NIC","PAN","BRA","COL","ECU","PRY","PER","AUT","BEL","DNK",
  "FIN","FRA","DEU","ITA","NLD","NOR","SWE","CHE","GBR","CAN",
  "TTO","USA","ARG","CHL","URY","VEN","AUS","NZL")
...
```{r}
wealth1_1 = run_ps(ineq1 |> filter(year %in% 1995:2020), dataCols = 2:27); wealth1_1$clubs

wealth1_2 = run_ps(ineq2 |> filter(year %in% 1995:2020), dataCols = 2:27); wealth1_2$clubs

wealth1_3 = run_ps(ineq3 |> filter(year %in% 1995:2020), dataCols = 2:27); wealth1_3$clubs
```

## Wealth (top 10%)

```{r}
wealth10_1 = run_ps(ineq1 |> filter(year %in% 1995:2020),
                    measure = "sh_wealth_top10",
                    dataCols = 2:27); wealth10_1$clubs

wealth10_2 = run_ps(ineq2 |> filter(year %in% 1995:2020),
                    measure = "sh_wealth_top10",
                    dataCols = 2:27); wealth10_2$clubs

wealth10_3 = run_ps(ineq3 |> filter(year %in% 1995:2020),
                    measure = "sh_wealth_top10",
                    dataCols = 2:27); wealth10_3$clubs
```
...
```{r}
gini_1 = run_ps(ineq1 |> filter(year %in% 2004:2013, !(code %in% c("MMR", "COD"))),
                                measure = "pip_gini",
                                dataCols = 2:11); gini_1$clubs
...
gini_3 = run_ps(ineq3 |> filter(year %in% 2004:2013,
                                !(code %in% c("HKG", "JAM", "JPN", "SGP", "NZL",
                                              "TTO", "VEN"))),
                                measure = "pip_gini",
                                dataCols = 2:11); gini_3$clubs
```
